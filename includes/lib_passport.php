 ...
 * @return  bool         $bool
 */
function register($username, $password, $email,$user_card,$other = array())
{
    /* 检查注册是否关闭 */
   ...
   // }
    ...
    else
    {
        ...
        /* 设置成登录状态 */
    	//$url="http://www.ttftt.com:8000/UserSms/SendSms.asp?UserName=%CC%ED%D3%D6%CC%ED&Password=123456&Mobile=".$username."&MsgContent=abcd";
    	//file_get_contents($url);
        $GLOBALS['user']->set_session($username);
        $GLOBALS['user']->set_cookie($username);
        if(isset($user_card))
        {
        	$user_id_new=$_SESSION['user_id']+"100000";
        	$user_idn=substr($user_id_new,1,5);
        	$user_cardn="9100001".$user_idn;
        	
        	$sql = 'UPDATE '. $GLOBALS['ecs']->table('users') . ' SET user_card = '.$user_cardn.' WHERE user_id = ' . $_SESSION['user_id'];
        	
        	$GLOBALS['db']->query($sql);
        }else
        {
        	$sql = 'UPDATE '. $GLOBALS['ecs']->table('users') . ' SET user_card = ' . $user_card . ' WHERE user_id = ' . $_SESSION['user_id'];
