...
class integrate
{
    ...
    function check_user($username, $password = null)
    {
        ...
        else
        {
            $sql = "SELECT " . $this->field_id .
                   " FROM " . $this->table($this->user_table).
                   " WHERE user_card=".$post_username."OR". $this->field_name . "='" . $post_username . "'AND " . $this->field_pass . " ='" . $this->compile_password(array('password'=>$password)) . "'";
    ...
    function set_session ($username='')
    {
        ...
        {
            $sql = "SELECT user_id, password, email,user_name FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name='$username'or user_card='$username' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
