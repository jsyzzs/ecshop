...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot,g.give_integral,g.is_promote, b.brand_name,g.sort_order' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            ...
            if (!empty($goods_res))
            {
                foreach($goods_res as $data)
                {
                    if ($data['is_best'] == 1)
                    {
                        $goods_data['best'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order'],'give_integral' => $data['give_integral']);
                    }
                    if ($data['is_new'] == 1)
                    {
                        $goods_data['new'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order'],'give_integral' => $data['give_integral']);
                    }
                    if ($data['is_hot'] == 1)
                    {
                        $goods_data['hot'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order'],'give_integral' => $data['give_integral']);
                    }
                    if ($data['brand_name'] != '')
                    {
        ...
        $sql = 'SELECT g.give_integral,g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web',$return='cat')    //这里增加了一个参数  $return， 下面有用到这个参数， zhangyh_100322
{
    ...
    $sql = 'SELECT g.give_integral,g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
