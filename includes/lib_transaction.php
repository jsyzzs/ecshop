       ...
       if ($GLOBALS['db']->query($sql))
        {
            ...
            if ($orders['user_id'] > 0)
            {
            	...
            	log_account_change($orders['user_id'], 0, 0, intval($integral['rank_points']), intval($integral['custom_points']), sprintf($_LANG['order_gift_integral'], $order['order_sn']));
            
            	/* 取得用户信息 
            	$user_aff = user_info($orders['user_id']);
            	
            	if($user_aff['pay_points']>$shop_jifen){
            		$i=$user_aff['pay_points'];
            	while ($i>=$shop_jifen) {
            		$i=$i-$shop_jifen;
            		$sqlu="UPDATE".$GLOBALS['ecs']->table('users')." SET pay_points='".$i."'WHERE user_id='".$orders['user_id']."'" ;
            		$GLOBALS['db']->query($sqlu);
            		$times= date('Y-m-d H:i:s');
            		$line=array();
					$line['in_time']=$times;
					$line['user_id']=$orders['user_id'];
					$line['out_time']='';
					$line['is_use']='0';
					$line['groud']=$_CFG['shop_jifen'];
					$line['order_id']=$order['order_sn'];
					...
					if(is_int($slnum/5) ){
						...
						if($n==1){
							...
							$sqln="SELECT user_id FROM".$GLOBALS['ecs']->table('line').' LIMIT ' . $n-1 . ', ' . $n;
						}
						$sln = $GLOBALS['db']->GetRow($sqln);
						$usern = user_info($sln['user_id']);
						$in=$usern['user_money']+$_CFG['shop_jifen'];
