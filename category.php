...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('comment') . " where comment_type=0 and id_value ='$goods_id'");
     	...
     	$pingfen=$GLOBALS['db']->getOne("SELECT IFNULL(AVG(comment_rank), 0) AS comment_rank FROM " . $GLOBALS['ecs']->table('comment') . " where  comment_type=0 and status =1 and id_value ='$goods_id'");  
     	$arr[$row['goods_id']]['pingfen']      = ceil($pingfen) == 0 ? 5 : ceil($pingfen);
     	
     	$arr[$row['goods_id']]['buy_nums']      =  get_buy_sum($row['goods_id']);
        ...
        $arr[$row['goods_id']]['link_pre']= get_linked_articles($row['goods_id']);
        $d = get_linked_articles($row['goods_id']);
        $arr[$row['goods_id']]['title'] =  $d[0]['title'];
		...
		$properties['pro'] = $arrs;
        $arr[$row['goods_id']]['properties']= $arrs;  // 商品属性
...
{
    $sql = 'SELECT a.article_id, a.title, a.file_url,a.description, a.open_type, a.add_time ' .
