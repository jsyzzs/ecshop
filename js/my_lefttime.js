...
function showtime(obj)
{
  ...
  if (dateLeft > 0)
  {
    Temp = "<ul class='time'>剩余时间：</ul><div>"+hourZero+hourLeft+"</div><ul>"+_hour+"</ul><div>" + minuteZero + minuteLeft +"</div><ul>" + _minute + "</ul><div>" + secondZero + secondLeft + "</div>" ;
  }
  else
  {
    if (hourLeft > 0)
    {
      Temp = "<div>" + hourLeft + "</div><ul>" + _hour + "</ul><div>" + minuteZero + minuteLeft + "</div><ul>" + _minute + "</ul><div>" + secondZero + secondLeft + "</div>";
    }
    else
    {
      if (minuteLeft > 0)
      {
        Temp = "<div>" + minuteLeft + "</div><ul>" + _minute + "</ul><div>" + secondZero + secondLeft + "</div>";
      }
      else
      {
        if (secondLeft > 0)
        {
          Temp = "<div>" + secondLeft  + "</div>";
        }
        else
        {
          Temp = '';
