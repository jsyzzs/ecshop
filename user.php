...
$not_login_arr =
array('pd','paidui','act_action','activation','login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer');

/* 显示页面的action列表 */
$ui_arr = array('pd','paidui','act_action','activation','register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log','account_jf', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer');
...
if ($action == 'default')
{
    ...
    $smarty->assign('shop_jifen', $_CFG['shop_jifen']);
...
elseif ($action == 'act_action')
{
	...
	else
	{
		...
		$password = isset($_POST['password']) ? trim($_POST['password']) : '';
		$email    = isset($_POST['username']) ? trim($_POST['username']) : '';
		$user_card   = isset($_POST['user_card']) ? trim($_POST['user_card']) : '';
		$vip_num_pass    = isset($_POST['vip_num_pass']) ? trim($_POST['vip_num_pass']) : '';
		$other['msn'] = isset($_POST['extend_field1']) ? $_POST['extend_field1'] : '';
		$other['qq'] = isset($_POST['extend_field2']) ? $_POST['extend_field2'] : '';
		$other['office_phone'] = isset($_POST['extend_field3']) ? $_POST['extend_field3'] : '';
		$other['home_phone'] = isset($_POST['extend_field4']) ? $_POST['extend_field4'] : '';
		...
		if (register($username, $password, $email,$user_card,$other) !== false)
		{
			/*把新注册用户的扩展信息插入数据库*/
			$sql = 'SELECT id FROM ' . $ecs->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有自定义扩展字段的id
			$fields_arr = $db->getAll($sql);
...
elseif ($action == 'profile')
{
   ...
   // $smarty->display('user_transaction.dwt');
    ...
    $smarty->assign('consignee_list_cade', $consignee_list);
...
elseif ($action == 'paidui')
{
	...
	/*判断用户积分是否大于设定值*/
	$surplus_id=$user_id;
	$user_jifen = get_user_pay_points($surplus_id);
	$shop_jifen = $_CFG['shop_jifen'];	
	/* 变量初始化 */
	$surplus = array(
			'user_id'      => $user_id,
	...
	if($user_jifen>=$shop_jifen)
	{
	    $amount = isset($_POST['amount']) ? floatval($_POST['amount']) : 0;
	    if($amount<=0)
	    {
	    	show_message($_LANG['amount_gt_pay']);
	    }
		$n=$amount%$shop_jifen;
		$ii= floatval($amount/$shop_jifen);
		if($n>0)
		{
			show_message($_LANG['amount_gt_pay_no']);
		}else {
			if ($surplus['process_type'] == 1)
			{
				for ($i=1; $i<=$ii; $i++)
				{
				 		$user_aff = user_info($surplus_id);
						$user_jf=$user_aff['pay_points'];
				 
						$user_jf=$user_jf-$shop_jifen;
						$new_jifen='-'.$shop_jifen;
						$sqlu="UPDATE".$GLOBALS['ecs']->table('users')." SET pay_points='".$user_jf."'WHERE user_id='".$surplus_id."'" ;
						//$GLOBALS['db']->query($sqlu);
						log_account_change($surplus_id, 0, 0, 0, $new_jifen, "排队积分减少", 99);
						
						$times= date('Y-m-d H:i:s');
						$line=array();
						$line['in_time']=$times;
						$line['user_id']=$surplus_id;
						$line['out_time']='';
						$line['is_use']='0';
						$line['groud']='50';
						$line['order_id']='88888888';
						$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('line'), $line, 'INSERT');
						
						$sqls = "SELECT * FROM ".$GLOBALS['ecs']->table('line');
						$sle = $GLOBALS['db']->query($sqls);
						$slnum = $GLOBALS['db']->num_rows($sle);
						if(is_int($slnum/5) )
						{
							$n=floatval($slnum/5);
							$nn=floatval($n-1);
							if($n==1)
							{
								$sqln="SELECT user_id FROM".$GLOBALS['ecs']->table('line').' LIMIT 0,' . $n;
							}else{
								$sqln="SELECT user_id FROM".$GLOBALS['ecs']->table('line').' LIMIT ' .$nn. ', ' . $n;
							}
							$sln = $GLOBALS['db']->GetRow($sqln);
							$usern = user_info($sln['user_id']);
							$in=$usern['user_money']+50;
							
							$sqlum="UPDATE".$GLOBALS['ecs']->table('users')." SET user_money='".$in."'WHERE user_id='".$usern['user_id']."'" ;
							log_account_change_new($usern['user_id'], 50, 0, 0, 0, "排队金额", 99);
							$GLOBALS['db']->query($sqlum);
			...
			show_message($_LANG['amount_gt_pay_ok']);
		}
...
elseif ($action == 'act_account')
{
    ...
    {
        /* 判断是否有足够的余额的进行退款的操作 */
        $sur_amount = get_user_pay_points($user_id);
        ...
        if($nn>0)
        {
        	...
        	$get_n=$amount/$shop_jifen;
        	$get_Money=$get_n*8;
        	
        }
     	
		$sur_money = get_user_surplus($user_id);
		
        //插入会员账目明细
        $new_amount = '-'.$amount;//消费积分
        $surplus['amount_new']=$sur_amount-$amount;//积分变动
        $surplus['payment'] = '积分兑换金币';
		$surplus['user_id'] = $user_id;
		$surplus['new_money'] =$sur_money+$getMoney;//金币变动
        //$surplus['rec_id']  = update_user_jifen($surplus);
        log_account_change($user_id, $get_Money, 0, 0, 0, "兑换金币", 99);
        $surplus['rec_id']  = log_account_change($user_id, 0, 0, 0, $new_amount, "兑换金币积分减少", 99);
        /* 如果成功提交 */
        if ($surplus['rec_id']==TRUE)
        {
